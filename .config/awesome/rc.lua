...
vicious.register(memwidget, vicious.widgets.mem,
  function(widget, args)
    ...
    local usingmb = args[2]
    local str = "MEM:<span weight='bold' color='%s'>%d%%(%dMB)</span>|"

    if percent < 70 then
      return string.format(str, "#00CC00", percent, usingmb)
    elseif percent < 80 then
      return string.format(str, "#CCCC00", percent, usingmb)
    elseif percent < 90 then
      return string.format(str, "#CC6600", percent, usingmb)
    else
      return string.format(str, "#CC0000", percent, usingmb)
    end
  end,
  13)

cpuwidget = wibox.widget.textbox()
--vicious.register(cpuwidget, vicious.widgets.cpu, "CPU:$1% $2% $3% $4%|")
vicious.register(cpuwidget, vicious.widgets.cpu,
  function(widget, args)
    ...
    for i=1, cores do
      percent = percent + args[i]
    end

    return "CPU:" .. math.floor(percent/cores + 0.5) .. "%|"
  end
  )

function decorate_percent_str(percent)
  local str = "<span weight='bold' color='%s'>%d" .. "%%</span>|"
...
vicious.register(batwidget0, vicious.widgets.bat,
  function(widget, args)
    ...
    return status_str .. "BAT0:" .. decorate_percent_str(args[2] + 0)
  end,
  10, "BAT0")
...
vicious.register(batwidget1, vicious.widgets.bat,
  function(widget, args)
    if args[2] == 0 then
      ...
      return status_str .. "BAT1:" .. decorate_percent_str(args[2] + 0)
    end
  end,
  10, "BAT1")
...
lantooltip:add_to_object(lanwidget)
vicious.register(lanwidget, vicious.widgets.net,
  function(widget, args)
    local state = args["{ens9 carrier}"]
    local label = '無'
    local color = '#CC0000'
    ...
    if state ~= nil and state ~= 0 then
      ...
      label = '接'
      color = "#009FE5"
      --lantooltip:set_text('carrier: '..state)
      return ("LAN:<span color='%s'>%s</span>|"):format(color, label)
    end
...
globalkeys = awful.util.table.join(
    ...
    -- Screenshot
    awful.key({ modkey, "Shift"   }, "3", function() awful.util.spawn("shutter -e 'mv $f ~/screenshots/ 2> /dev/null'") end)
)
...
clientkeys = awful.util.table.join(
    ...
    -- ここから独自設定
    -- awful.key({ modkey, "Shift" }, "Return",      function (c)  awful.util.spawn("xscreensaver-command -lock") end)
)
